  ...
  <div class="resource-page">
    ...
    <div v-else class="resources-grid">
      <div v-for="resource in filteredResources" :key="resource.id" class="resource-card">
        ...
        <div class="resource-actions">
          <button v-if="resource.status === 'FAILED' || resource.status === 'CANCELLED' || (!resource.status || resource.status === '')" 
                  class="btn btn-primary" 
                  @click="downloadResource(resource.id)">
            <span class="material-icons">download</span> 下载
          </button>
          <button v-if="resource.status === 'DOWNLOADING' || resource.status === 'PENDING'" 
  ...
  setup() {
    ...
    const loadResources = async () => {
      ...
      try {
        ...
        // 更新资源数据
        resources_data.value = newResources;
        ...
        console.log(`已加载 ${newResources.length} 个资源`);
      } catch (err) {
    ...
    const addResource = async () => {
      try {
        ...
        if (response.data) {
          ...
          resources_data.value.push(newResourceData);
        } else {
        ...
        showToast('资源已添加，点击下载按钮开始下载', 'success')
      } catch (err) {
    ...
    const downloadResource = async (resourceId) => {
      try {
        ...
        if (targetResource.status === 'DOWNLOADING' || targetResource.status === 'PENDING') {
          showToast('该资源正在等待下载或下载中', 'info');
          return;
        }
        ...
        if (resourceIndex !== -1) {
          ...
          // 同时更新状态历史记录，以便WebSocket更新可以正确处理
          resourceStatusHistory[resourceId] = 'PENDING';
        }
    ...
    const cancelDownload = async (resourceId) => {
      try {
        ...
        // 如果资源只是处于PENDING状态但实际上后端还没开始下载
        if (targetResource.status === 'PENDING') {
          // 直接本地更新状态，无需调用后端API
          const resourceIndex = resources_data.value.findIndex(r => r.id === resourceId);
          if (resourceIndex !== -1) {
            const updatedResource = {
            ...
            resources_data.value.splice(resourceIndex, 1, updatedResource);
            resourceStatusHistory[resourceId] = 'CANCELLED';
    ...
    const setupWebSocket = () => {
      ...
      const wsUrl = `${wsProtocol}//${url.host}/ws/resources`;
      ...
      ws.value.onmessage = (event) => {
        try {
          ...
          if (data.type === 'resource_update' && data.resource_id) {
            ...
            if (resourceIndex !== -1) {
              ...
              if (data.current_file && data.total_files) {
                // 如果收到了文件下载信息，计算更精确的进度
                const fileIndex = data.current_file;
                const totalFiles = data.total_files;
                const filePercent = Math.min(Math.max(0, fileIndex / totalFiles * 100), 100);
                fileProgress = filePercent;
                
                console.log(`文件下载进度: ${fileIndex}/${totalFiles} (${filePercent.toFixed(2)}%)`);
              }
              ...
              if (data.status !== undefined) {
                updatedResource.status = data.status;
              }
              
              // 更新进度
              if (data.progress !== undefined) {
                updatedResource.progress = data.progress;
              } else if (fileProgress !== null) {
                updatedResource.progress = fileProgress;
              }
              
              // 更新错误信息
              if (data.error_message !== undefined) {
                updatedResource.error_message = data.error_message;
              }
              
              // 如果有完整的资源信息，使用它
              if (data.resource) {
                Object.assign(updatedResource, data.resource);
              }
              
              // 如果是下载中状态但进度为0，设置一个小的初始进度以显示动态
              if (updatedResource.status === 'DOWNLOADING' && (updatedResource.progress === 0 || updatedResource.progress === undefined)) {
                updatedResource.progress = 0.5;
              }
              
              // 确保进度值在有效范围内
              if (updatedResource.progress !== undefined) {
                updatedResource.progress = Math.min(Math.max(0, updatedResource.progress), 100);
              }
              
              // 替换资源数组中的对象，确保Vue能够检测到变化
              resources_data.value.splice(resourceIndex, 1, updatedResource);
              
              console.log(`已更新资源 ${data.resource_id} 状态: ${oldStatus} -> ${updatedResource.status}, 进度: ${updatedResource.progress}%`);
              
              // 如果资源状态从下载中变为已完成或失败，强制刷新获取完整信息
              if (oldStatus === 'DOWNLOADING' && 
                  (updatedResource.status === 'COMPLETED' || updatedResource.status === 'FAILED')) {
                console.log('资源下载状态已变更，强制刷新页面');
                setTimeout(() => {
                  loadResources();
                }, 1000); // 延迟1秒刷新，确保后端状态已完全更新
              }
            } else {
          ...
          } else if (data.type === 'resource_file_progress' && data.resource_id) {
            // 处理文件下载进度更新
            const resourceId = data.resource_id;
            ...
            // 计算进度百分比
            const progressPercent = Math.min(Math.max(0, (fileIndex / totalFiles) * 100), 100);
            
            console.log(`资源 ${resourceId} 文件下载进度: ${fileIndex}/${totalFiles} (${progressPercent.toFixed(2)}%), 文件: ${fileName}`);
      ...
      ws.value.onclose = (event) => {
        ...
        // 如果不是主动关闭，启动重连
        if (event.code !== 1000) {
          console.log('WebSocket异常关闭，将尝试重新连接...');
          startReconnectTimer();
        }
      };
    };
    ...
    const handleStatusChange = (resourceId, newStatus, oldStatus, resourceName) => {
      ...
      if (newStatus !== oldStatus) {
        console.log(`资源状态变化: ${resourceName} (${resourceId}) ${oldStatus} -> ${newStatus}`);
        
        if (newStatus === 'COMPLETED') {
          showToast(`${resourceName || '资源'} 下载完成！`, 'success', 5000)
          // 延迟刷新资源列表，确保状态同步
          setTimeout(() => {
            loadResources()
          }, 500);
        } else if (newStatus === 'FAILED') {
          showToast(`${resourceName || '资源'} 下载失败，请检查详情`, 'error', 8000)
          // 延迟刷新资源列表
          setTimeout(() => {
            loadResources()
          }, 500);
        } else if (newStatus === 'DOWNLOADING' && (!oldStatus || oldStatus === 'PENDING')) {
          showToast(`${resourceName || '资源'} 开始下载`, 'info', 3000)
        } else if (newStatus === 'CANCELLED') {
          showToast(`${resourceName || '资源'} 下载已取消`, 'info', 3000)
        }
      }
    }
      ...
      refreshTimer.value = setInterval(async () => {
        if (!loading.value) {
          ...
          try {
            ...
            // 更新状态历史记录
            resources_data.value.forEach(resource => {
              if (!resourceStatusHistory[resource.id]) {
                resourceStatusHistory[resource.id] = resource.status;
              }
            });
          } catch (error) {
