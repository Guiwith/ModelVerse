        ...
        <div class="modal-body">
          ...
          <div class="form-group">
            ...
            <select 
              ...
              class="form-control"
              required
              :disabled="config.training_type === 'PRETRAIN'"
            >
            ...
            <small v-if="config.training_type === 'PRETRAIN'" class="helper-text">
              预训练模式不需要选择基础模型，将从头开始训练
            </small>
          </div>
            ...
            <div class="advanced-config">
              ...
              <div class="config-section">
                ...
                <div v-if="['FSDP', 'FSDP_LORA'].includes(config.training_type)" class="config-section nested-section">
                  ...
                  <div class="config-item">
                    <label>分片策略</label>
                    <select v-model="fsdpSettings.sharding_strategy" class="form-control">
                  ...
                  <div class="config-item">
                    <label>启用前向预取</label>
                    <input type="checkbox" v-model="fsdpSettings.forward_prefetch" class="form-check" />
                  </div>
                  <div class="config-item">
                    <label>Transformer层类</label>
                    <input type="text" v-model="fsdpSettings.transformer_layer_cls" class="form-control" />
                  </div>
                </div>
                ...
                <div v-if="config.training_type === 'FSDP_LORA'" class="config-section nested-section">
                  ...
                  <div class="config-item">
                    <label>LoRA秩(r)</label>
                    <input type="number" v-model="peftSettings.lora_r" class="form-control" />
                  </div>
                  <div class="config-item">
                    <label>LoRA缩放(alpha)</label>
                    <input type="number" v-model="peftSettings.lora_alpha" class="form-control" />
                  </div>
                  <div class="config-item">
                    <label>LoRA丢弃率</label>
                    <input type="number" v-model="peftSettings.lora_dropout" step="0.01" class="form-control" />
                  </div>
                </div>
...
export default {
  ...
  setup() {
    ...
    // 确保FSDP设置总是有效的计算属性
    const fsdpSettings = computed(() => {
      // 如果config.fsdp不存在，则返回默认值
      return config.value.fsdp || {
        sharding_strategy: 'HYBRID_SHARD',
        forward_prefetch: true,
        auto_wrap_policy: 'TRANSFORMER_BASED_WRAP',
        transformer_layer_cls: 'LlamaDecoderLayer',
    ...
    // 确保LoRA设置总是有效的计算属性
    const peftSettings = computed(() => {
      // 如果config.peft不存在，则返回默认值
      return config.value.peft || {
        lora_r: 8,
        lora_alpha: 16,
        lora_dropout: 0.0,
        lora_target_modules: ["q_proj", "v_proj"]
    ...
    const createTask = async () => {
      try {
        // 确保fsdp和peft设置从计算属性同步回config对象
        if (['FSDP', 'FSDP_LORA'].includes(config.value.training_type)) {
          config.value.fsdp = { ...fsdpSettings.value };
        }
        
        if (config.value.training_type === 'FSDP_LORA') {
          config.value.peft = { ...peftSettings.value };
        }
        ...
        const taskData = {
          ...
          config_params: {
            ...
            training_type: config.value.training_type
          } as any
        }
        
        // 对于FSDP和FSDP_LORA添加特定配置
        if (['FSDP', 'FSDP_LORA'].includes(config.value.training_type)) {
          taskData.config_params.fsdp = config.value.fsdp;
        }
        
        // 对于FSDP_LORA添加PEFT配置
        if (config.value.training_type === 'FSDP_LORA') {
          taskData.config_params.peft = config.value.peft;
        }
    ...
    const updateTrainerType = () => {
      switch(config.value.training_type) {
        case 'DPO':
          ...
          config.value.model.load_pretrained_weights = true
          break;
        case 'PRETRAIN':
          config.value.training.trainer_type = 'TRL_SFT'
          config.value.model.load_pretrained_weights = false
          
          // 为预训练模式设置默认基础模型（如果不需要基础模型）
          if (!newTask.value.base_model_id) {
            // 选择第一个可用模型作为形式上的基础模型
            if (availableModels.value.length > 0) {
              newTask.value.base_model_id = availableModels.value[0].id
            }
          }
          break;
        case 'SFT':
